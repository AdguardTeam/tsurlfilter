...
describe('TestEmptyNetworkEngine', () => {
    it('works if empty engine is ok', () => {
        const engine = new NetworkEngine(createTestRuleStorage(1, []), new ByteBuffer());
        engine.finalize();
...
describe('TestMatchAllowlistRule', () => {
    it('works if it finds simple allowlist rule', () => {
        ...
        const engine = new NetworkEngine(createTestRuleStorage(1, [rule, exceptionRule]), new ByteBuffer());
        engine.finalize();
...
describe('TestMatchImportantRule', () => {
    ...
    const engine = new NetworkEngine(createTestRuleStorage(1, [r1, r2, r3]), new ByteBuffer());
    engine.finalize();
    let request;
    let result;
...
describe('TestMatchSourceRule', () => {
    it('works if it finds rule for source url', () => {
        ...
        const engine = new NetworkEngine(createTestRuleStorage(1, [rule]), new ByteBuffer());
        engine.finalize();
