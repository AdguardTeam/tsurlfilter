...
describe('NetworkEngine', () => {
    describe('TestEmptyNetworkEngine', () => {
        it('works if empty engine is ok', () => {
            const engine = NetworkEngine.create(createTestRuleStorage(1, []), new ByteBuffer());
            engine.finalize();
    ...
    describe('TestMatchAllowlistRule', () => {
        it('works if it finds simple allowlist rule', () => {
            ...
            const engine = NetworkEngine.create(createTestRuleStorage(1, [rule, exceptionRule]), new ByteBuffer());
            engine.finalize();
    ...
    describe('TestMatchImportantRule', () => {
        ...
        beforeAll(() => {
            engine = NetworkEngine.create(createTestRuleStorage(1, [r1, r2, r3]), new ByteBuffer());
            engine.finalize();
        });
    ...
    describe('TestMatchSourceRule', () => {
        it('works if it finds rule for source url', () => {
            ...
            const engine = NetworkEngine.create(createTestRuleStorage(1, [rule]), new ByteBuffer());
            engine.finalize();
    ...
    describe('Test $domain modifier semantics', () => {
        ...
        beforeAll(() => {
            engine = NetworkEngine.create(createTestRuleStorage(1, [rule]), new ByteBuffer());
            engine.finalize();
        });
