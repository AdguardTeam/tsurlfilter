...
describe('Trie Lookup Table Tests', () => {
    it('adds rule to look up table', () => {
        ...
        expect(table.addRule(tokenize('http://p')!, 0)).toBeFalsy();
        expect(table.getRulesCount()).toBe(0);

        expect(table.addRule(tokenize('shortcut')!, 0)).toBeTruthy();
        expect(table.getRulesCount()).toBe(1);

        expect(table.addRule(tokenize('path')!, 0)).toBeTruthy();
        expect(table.getRulesCount()).toBe(2);

        expect(table.addRule(tokenize('https://domain.com')!, 0)).toBeTruthy();
        expect(table.getRulesCount()).toBe(3);

        // Rule shortcut is too short
        expect(table.addRule(tokenize('aa$app=com.mobile')!, 0)).toBeFalsy();
        expect(table.getRulesCount()).toBe(3);
    });
