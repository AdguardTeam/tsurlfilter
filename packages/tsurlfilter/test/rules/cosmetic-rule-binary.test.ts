...
describe('CosmeticRule binary path parity', () => {
    ...
    it.each([
        // uBO scriptlet (##+js) — must route through the AST path.
        'example.com##+js(set, atob, noopFunc)',
        // ABP snippet (#$# without a declaration block).
        'example.org#$#abort-on-property-read alert',
        // ADG CSS injection (#$# with a declaration block).
        'example.com#$#.ad { color: red }',
        // ADG HTML filtering ($$).
        'example.com$$div[id="ad"]',
    ])('createFromReader rejects non-binary cosmetic kinds: %s', (rule) => {
        const { ctx } = pipeline.parseStructural(rule);
        const reader = new CosmeticRuleDataReader(ctx, 0);
        expect(() => CosmeticRule.createFromReader(reader, rule, 0)).toThrow(/not supported by the binary path/);
    });
});
