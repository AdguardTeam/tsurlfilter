...
describe('NetworkRule binary path parity', () => {
    ...
    it('rejects host-shaped input with a space in the pattern', () => {
        const { ctx } = pipeline.parseStructural('127.0.0.1 example.com');
        const reader = new NetworkRuleDataReader(ctx, 0);
        expect(() => NetworkRule.createFromReader(reader, '127.0.0.1 example.com', 0))
            .toThrow(/host rule/);
    });
});
