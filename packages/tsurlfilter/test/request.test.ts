...
describe('Creating request', () => {
    it('works if simple request is parsed properly', () => {
        const request = new WebRequest('http://example.org/', null, RequestType.Other);
    ...
    it('works if source URL is parsed properly', () => {
        const request = new WebRequest('http://example.org/', 'http://sub.example.org', RequestType.Other);
        expect(request.url).toEqual('http://example.org/');
        expect(request.hostname).toEqual('example.org');
        expect(request.domain).toEqual('example.org');
    ...
    it('works if eTLD is parsed properly', () => {
        const request = new WebRequest('http://example.org.uk/', 'http://sub.example.org.uk', RequestType.Other);
    ...
    it('works if third-party is detected properly', () => {
        const request = new WebRequest('http://example.org.uk/', 'http://sub.example.org', RequestType.Other);
        expect(request.url).toEqual('http://example.org.uk/');
        expect(request.hostname).toEqual('example.org.uk');
        expect(request.domain).toEqual('example.org.uk');
        expect(request.sourceUrl).toEqual('http://sub.example.org');
    ...
    it('detects third-party properly for IP domains', () => {
        const request = new WebRequest('https://example.org', 'https://1.2.3.4/', RequestType.Other);
        expect(request.sourceHostname).toEqual('1.2.3.4');
        expect(request.thirdParty).toEqual(true);

        const request2 = new WebRequest('https://1.2.3.4/', 'https://1.2.3.4/', RequestType.Other);
        expect(request2.thirdParty).toEqual(false);
    });

    it('parses subdomains', () => {
        const request = new WebRequest('http://sub.sub.example.org/part', 'http://sub.example.org', RequestType.Other);
    ...
    it('parses subdomains with complex tld', () => {
        const request = new WebRequest('http://sub.sub.example.org.uk/part', 'http://sub.example.org', RequestType.Other);
    ...
    it('parses domains with complex public suffixes', () => {
        ...
        const request = new WebRequest('https://www.city.toyota.aichi.jp/part', 'https://www.city.toyota.aichi.jp/', RequestType.Other);
        expect(request.subdomains.sort())
            .toEqual(['www.city.toyota.aichi.jp', 'city.toyota.aichi.jp', 'toyota.aichi.jp', 'aichi.jp', 'jp'].sort());
    });

    it('parses subdomains for localhost', () => {
        const request = new WebRequest('http://localhost', 'http://localhost.test', RequestType.Other);
    ...
    it('handles urls', () => {
        let f = () => new WebRequest('', 'example.com', RequestType.Other);
        expect(f).toThrow(TypeError);
        expect(f).toThrow(/Invalid request url:/);

        // @ts-ignore
        f = () => new WebRequest(undefined, 'example.com', RequestType.Other);
        expect(f).toThrow(TypeError);
        expect(f).toThrow(/Invalid request url:/);
