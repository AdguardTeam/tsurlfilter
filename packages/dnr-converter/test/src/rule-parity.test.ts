...
describe('text roundtrip parity', () => {
    ...
    it.each(corpusRules)('dnr-converter text equals source for unconverted rules: %s', (ruleText) => {
        const [dnrRule] = Rule.createFromText(1, 0, ruleText);
        // Rules whose modifiers need no AG-syntax conversion render unchanged.
        // The basic and single-flag-modifier rules in the corpus are stable.
        if (/^(?:@@)?\|\|[^$]+\^\$?(?:third-party|important|script|image)$/.test(ruleText)) {
