...
describe('RulesConverter', () => {
    ...
    describe('checkLimitations lazy indexes', () => {
        beforeEach(() => {
            vi.clearAllMocks();
        });
        ...
        it('AC1: should not build the indexes when no limits are set', () => {
            const declarativeRules = [
                createDeclarativeRule(1, 'example.com'),
                createDeclarativeRule(2, 'test.com'),
            ];
            ...
            const groupByDeclarativeRuleIdSpy = vi.spyOn(RulesConverter as any, 'groupByDeclarativeRuleId');

            // @ts-expect-error Accessing private method for testing
            const result = RulesConverter.checkLimitations(converted);

            expect(groupByDeclarativeRuleIdSpy).not.toHaveBeenCalled();

            // Output is still correct (unchanged from input)
            expect(result.declarativeRules).toEqual(declarativeRules);
            expect(result.sourceMapValues).toEqual(sourceMapValues);
            expect(result.errors).toEqual(errors);
        });

        it('AC1: should not build the indexes when limits are set but not exceeded', () => {
            const declarativeRules = [
                createDeclarativeRule(1, 'example.com'),
                createDeclarativeRule(2, 'test.com'),
            ];
            ...
            const result = RulesConverter.checkLimitations(
                converted,
                5, // maxNumberOfRules - not exceeded
                3, // maxNumberOfUnsafeRules - not exceeded
                2, // maxNumberOfRegexpRules - not exceeded
            );
        ...
        it('AC2/AC3: should build each index exactly once and cache it when a rule is truncated', () => {
            const declarativeRules = [
                createDeclarativeRule(1, 'example.com'),
                createDeclarativeRule(2, 'test.com'),
                createDeclarativeRule(3, 'another.com'),
            ...
            const errors = [createConversionError(1), createConversionError(3)];
            const converted = createConvertedRules(declarativeRules, sourceMapValues, errors);
            ...
            // groupByDeclarativeRuleId is called once per Lazy loader (sources
            // and errors), each cached thereafter — 2 calls total.
            expect(groupByDeclarativeRuleIdSpy).toHaveBeenCalledTimes(2);

            // Truncation still works correctly
            expect(result.declarativeRules).toHaveLength(2);
            expect(result.declarativeRules.map((r) => r.id)).toEqual([1, 2]);
            expect(result.sourceMapValues).toHaveLength(2);
            expect(result.sourceMapValues.map((s) => s.declarativeRuleId)).toEqual([1, 2]);
            // Error for the truncated rule 3 is removed; error for rule 1 stays
            expect(result.errors).toHaveLength(1);
            expect(result.errors[0]).toBe(errors[0]);
        });

        it('AC2/AC3: should build each index exactly once even when both truncation blocks run', () => {
            const declarativeRules = [
                createDeclarativeRule(1, 'example.com'), // non-regex
                createDeclarativeRule(2, '^https?:\\/\\/regex1\\.com\\/.*$', true), // regex
                createDeclarativeRule(3, '^https?:\\/\\/regex2\\.com\\/.*$', true), // regex
            ...
            const result = RulesConverter.checkLimitations(
                converted,
                4, // maxNumberOfRules - not exceeded (no truncation here)
                undefined, // maxNumberOfUnsafeRules
                1, // maxNumberOfRegexpRules - truncates rule 3 (2nd regex)
            );

            // Only the regexp block truncates, so groupByDeclarativeRuleId is
            // called once per Lazy loader (sources and errors) — 2 calls total.
            expect(groupByDeclarativeRuleIdSpy).toHaveBeenCalledTimes(2);
    ...
    describe('groupByDeclarativeRuleId', () => {
        it('should group sources by declarative rule id preserving order', () => {
            const sourceMapValues = [
                createSource(1, 10, 100),
                createSource(2, 20, 200),
                createSource(1, 11, 100),
            ];

            // @ts-expect-error Accessing private method for testing
            const index = RulesConverter.groupByDeclarativeRuleId(
                sourceMapValues,
                (s: Source) => s.declarativeRuleId,
            );
            ...
            expect(index.get(2)).toEqual([createSource(2, 20, 200)]);
            // Map key insertion order follows first appearance in the array
            expect(Array.from(index.keys())).toEqual([1, 2]);
        });

        it('should group conversion errors by declarative rule id preserving order', () => {
            // createConversionError returns ConversionError but actually
            // produces InvalidDeclarativeRuleError instances — cast for the
            // id accessor used in production (checkLimitations narrows the
            // array the same way before calling groupByDeclarativeRuleId).
            const errors = [
                createConversionError(1),
                createConversionError(3),
                createConversionError(1),
            ] as InvalidDeclarativeRuleError[];

            // @ts-expect-error Accessing private method for testing
            const index = RulesConverter.groupByDeclarativeRuleId(
                errors,
                (e) => e.declarativeRule.id,
            );

            expect(index.get(1)).toHaveLength(2);
            expect(index.get(1)![0]).toBe(errors[0]);
            expect(index.get(1)![1]).toBe(errors[2]);
            expect(index.get(3)).toEqual([errors[1]]);
            expect(Array.from(index.keys())).toEqual([1, 3]);
        });

        it('should return an empty map for an empty list', () => {
            // @ts-expect-error Accessing private method for testing
            const index = RulesConverter.groupByDeclarativeRuleId([], () => 0);
