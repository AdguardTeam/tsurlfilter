...
describe('RulesConverter', () => {
    ...
    describe('checkLimitations lazy indexes', () => {
        ...
        it('AC1: should not build the indexes when no limits are set', () => {
            ...
            const buildSourcesIndexSpy = vi.spyOn(RulesConverter as any, 'buildSourcesIndex');
            const buildErrorsIndexSpy = vi.spyOn(RulesConverter as any, 'buildErrorsIndex');

            // @ts-expect-error Accessing private method for testing
            const result = RulesConverter.checkLimitations(converted);

            expect(buildSourcesIndexSpy).not.toHaveBeenCalled();
            expect(buildErrorsIndexSpy).not.toHaveBeenCalled();
        ...
        it('AC2/AC3: should build each index exactly once and cache it when a rule is truncated', () => {
            ...
            // Each builder runs exactly once (cached thereafter)
            expect(buildSourcesIndexSpy).toHaveBeenCalledTimes(1);
            expect(buildErrorsIndexSpy).toHaveBeenCalledTimes(1);
        ...
        it('AC2/AC3: should build each index exactly once even when both truncation blocks run', () => {
            ...
            // Only the regexp block truncates, so each builder runs once
            expect(buildSourcesIndexSpy).toHaveBeenCalledTimes(1);
            expect(buildErrorsIndexSpy).toHaveBeenCalledTimes(1);
    ...
    describe('buildSourcesIndex', () => {
        it('should group sources by declarative rule id preserving order', () => {
            ...
            // @ts-expect-error Accessing private method for testing
            const index = RulesConverter.buildSourcesIndex(sourceMapValues);
        ...
        it('should return an empty map for an empty source list', () => {
            // @ts-expect-error Accessing private method for testing
            const index = RulesConverter.buildSourcesIndex([]);
            expect(index.size).toBe(0);
        });
    });

    describe('buildErrorsIndex', () => {
        it('should group conversion errors by declarative rule id preserving order', () => {
            ...
            // @ts-expect-error Accessing private method for testing
            const index = RulesConverter.buildErrorsIndex(errors);
        ...
        it('should return an empty map for an empty error list', () => {
            // @ts-expect-error Accessing private method for testing
            const index = RulesConverter.buildErrorsIndex([]);
            expect(index.size).toBe(0);
        });
    });
