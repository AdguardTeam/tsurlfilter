...
describe('SourceMap', () => {
    ...
    describe('deserializeSources', () => {
        ...
        it('deserializes the legacy JSON-array format for backward compatibility', () => {
            // VLQ strings never start with '[', so a leading bracket
            // unambiguously identifies the old JSON-array format persisted by
            // previous @adguard/tsurlfilter versions (before the VLQ encoding).
            // It must be recovered transparently on an in-place upgrade rather
            // than throwing.
            const legacyJson = '[[1, 0, 1], [2, 10, 2]]';
            const result = SourceMap.deserializeSources(legacyJson);
            expect(result).toEqual([
                { declarativeRuleId: 1, sourceRuleIndex: 0, filterId: 1 },
                { declarativeRuleId: 2, sourceRuleIndex: 10, filterId: 2 },
            ]);
        });
    });
});
