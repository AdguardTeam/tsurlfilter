...
describe('Filter', () => {
    ...
    describe('getRuleByIndex()', () => {
        ...
        it('returns empty string for an out-of-range offset', async () => {
            const filter = new Filter(1, async () => '||a.com^');
            expect(await filter.getRuleByIndex(9999)).toBe('');
        });

        it('returns empty string for a mid-line (non-start) offset', async () => {
            const filter = new Filter(1, async () => '||a.com^');
            expect(await filter.getRuleByIndex(5)).toBe('');
        });
