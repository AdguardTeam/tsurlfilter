...
export class RulesConverter {
     ...
     */
    private static groupByDeclarativeRuleId<T>(
        items: T[],
        idAccessor: (item: T) => number,
    ): Map<number, T[]> {
    ...
    private static checkLimitations(
        ...
        const sourcesIndexLoader = new Lazy<Map<number, Source[]>>(() => (
            RulesConverter.groupByDeclarativeRuleId(sourceMapValues, (s) => s.declarativeRuleId)
        ));
        const errorsIndexLoader = new Lazy<Map<number, ConversionError[]>>(() => (
            RulesConverter.groupByDeclarativeRuleId(convertedRulesErrors, (e) => e.declarativeRule.id)
        ));
