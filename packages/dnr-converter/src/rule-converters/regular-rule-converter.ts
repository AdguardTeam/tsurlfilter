...
export class RegularRuleConverter {
    ...
    private static getCondition(rule: Rule): RuleCondition {
        ...
        if (hasExcludedResourceTypes) {
            // Shallow copy to drop reference linking (elements are primitive enum values)
     ...
     */
    // eslint-disable-next-line class-methods-use-this -- template method, overridden by subclasses
    protected groupConverted(converted: ConvertedRules): ConvertedRules {
        return converted;
    }
