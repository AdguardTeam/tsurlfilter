...
1. [MV3 specific limitations](#mv3_specific_limitations)
    ...
    1. [$removeheader, $csp](#mv3_specific_limitations__$removeheader__$csp)
...
### Problem 2
~~Previously, `$removeparam` rules with identical conditions were merged into a
single declarative rule (combining all param names into one `removeParams` array).
Applying `$badfilter` to one raw rule would cancel the entire merged declarative
rule, unintentionally removing other params from the output.~~

This problem no longer exists. Each `$removeparam` rule now produces its own
DNR rule with a param-aware `urlFilter` token (e.g.
`||testcases.adguard.com*^p1case1=`). As a result, `$badfilter` cancels exactly
the matching raw rule and its corresponding DNR rule, without affecting rules
...
<a name="mv3_specific_limitations__$removeheader__$csp"></a>
## $removeheader, $csp
Rules with `$removeheader` or `$csp` which contain the same conditions are
combined into one rule only within one filter but not across different filters.
Because of that, rules from different filters can conflict.

Note: `$removeparam` rules are **not** combined — each produces its own DNR
rule with a param-aware `urlFilter` token.
...
> ⚠️ Conversion errors: Declarative network rules with $header modifier cannot contain regex values
example 4
...
> ⚠️ Conversion errors: Network rule with explicitly enabled $popup modifier is not supported
...
> ⚠️ Conversion errors: "$removeparam is not compatible with $document" in the rule: "||example.com^$document,removeparam=test"
example 6
...
> ⚠️ Conversion errors: Network rule with "$content" modifier is not supported
...
> ⚠️ Conversion errors: Network rule with "$jsinject" modifier is not supported
...
> ⚠️ Conversion errors: Unsupported option "$stealth"
example 2
...
> ⚠️ Conversion errors: Network rule with "$urlblock" modifier is not supported
...
> ⚠️ Conversion errors: Unsupported option "$genericblock"
...
> ⚠️ Conversion errors: The use of additional parameters in $cookie (apart from $cookie itself) is not supported
example 2
...
> ⚠️ Conversion errors: Network allowlist rule with $cookie modifier is not supported
example 6
...
> ⚠️ Conversion errors: Network allowlist rule with $csp modifier is not supported
example 3
...
> ⚠️ Conversion errors: Network allowlist rule with $redirect modifier is not supported
example 4
...
> ⚠️ Conversion errors: Network rule with $redirect-rule modifier is not supported
...
> ⚠️ Conversion errors: Network allowlist rule with $removeheader modifier is not supported
...
> ⚠️ Conversion errors: Network rule with $removeparam modifier with negation or regexp is not supported
example 3
...
> ⚠️ Conversion errors: Network allowlist rule with $removeparam modifier is not supported
example 6
