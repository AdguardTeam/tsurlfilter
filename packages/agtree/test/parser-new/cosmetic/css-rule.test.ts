...
import type { CssRule } from '../../../src/nodes-new';
import { CssRuleParser } from '../../../src/parser-new/cosmetic/rule';

describe('CssRuleParser (parser-new)', () => {
    describe('CssRuleParser.parse - valid cases', () => {
        test('simple rule produces correct AST structure', () => {
            const input = 'div { color: red; }';
            const result: CssRule = CssRuleParser.parse(input);
        ...
        test('multiple selectors and declarations', () => {
            ...
            const result = CssRuleParser.parse(input);
        ...
        test('isLocIncluded: false omits start/end fields', () => {
            const input = 'div { color: red; }';
            const result = CssRuleParser.parse(input, { isLocIncluded: false });
        ...
        test('parsePrelude: false yields Raw prelude', () => {
            ...
            const result = CssRuleParser.parse(input, { parsePrelude: false });
        ...
        test('parseBlock: false yields Raw block', () => {
            const input = 'div { color: red; }';
            const result = CssRuleParser.parse(input, { parseBlock: false });
        ...
        test('baseOffset shifts all container node positions', () => {
            ...
            const result = CssRuleParser.parse(input, {}, baseOffset);
    ...
    describe('CssRuleParser.parse - error cases', () => {
        test('missing closing brace throws AdblockSyntaxError', () => {
            expect(() => CssRuleParser.parse('div { color: red;')).toThrow(AdblockSyntaxError);
        });
        ...
        test('empty selector throws AdblockSyntaxError', () => {
            expect(() => CssRuleParser.parse('   { color: red; }')).toThrow(AdblockSyntaxError);
        });
    });
});
