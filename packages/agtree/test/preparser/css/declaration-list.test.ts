/**
 * @file Preparser-level unit tests for `DeclarationListPreparser`.
 *
 * Each test tokenises a CSS declaration string, runs
 * `DeclarationListPreparser.preparse()`, then inspects the flat `Int32Array`
...
import {
    ...
    DECL_STRIDE,
    DeclarationListPreparser,
    DL_COUNT_OFFSET,
    DL_HEADER_SIZE,
} from '../../../src/preparser/css/declaration-list';
...
/**
 * Tokenise `source` and run `DeclarationListPreparser.preparse()`.
 *
...
function p(source: string, maxDeclarations?: number): void {
    ...
    DeclarationListPreparser.preparse(ctx, 0, ctx.tokenCount, DATA_OFFSET, maxDeclarations);
}

/**
 * Returns the declaration count from the header.
 *
 * @returns Number of declarations preparsed.
 */
...
describe('DeclarationListPreparser', () => {
