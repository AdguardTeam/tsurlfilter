...
import { AdblockSyntaxError, type CssSelectorList } from '../../../src';
import { CssSelectorListParser } from '../../../src/parser/cosmetic/css-selector/css-selector-list-parser';
import { CssSelectorListGenerator } from '../../../src/generator/cosmetic/css-selector/css-selector-list-generator';
import { CssSelectorListSerializer } from '../../../src/serializer/cosmetic/css-selector/css-selector-list-serializer';
import {
    CssSelectorListDeserializer,
} from '../../../src/deserializer/cosmetic/css-selector/css-selector-list-deserializer';

describe('CssSelectorListParser', () => {
    describe('CssSelectorListParser.parse - valid cases', () => {
        test.each<{ actual: string; expected: NodeExpectFn<CssSelectorList> }>([
            // tag name only
            {
                actual: 'div',
                expected: (context) => ({
            ...
            expect(CssSelectorListParser.parse(actual)).toEqual(
    ...
    describe('CssSelectorListParser.parse - invalid cases', () => {
        test.each<{ actual: string; expected: NodeExpectFn<AdblockSyntaxError> }>([
            ...
            const fn = vi.fn(() => CssSelectorListParser.parse(actual));
    ...
    describe('HtmlFilteringBodyGenerator.generate', () => {
        test.each<{ actual: string; expected: string }>([
            ...
            const ruleNode = CssSelectorListParser.parse(actual);

            if (ruleNode === null) {
                throw new Error(`Failed to parse '${actual}' as cosmetic rule`);
            }

            expect(CssSelectorListGenerator.generate(ruleNode)).toBe(expected);
        });
    });
    ...
    describe('serialize & deserialize', () => {
        test.each([
            ...
            await expect(input).toBeSerializedAndDeserializedProperly(
                CssSelectorListParser,
                CssSelectorListGenerator,
                CssSelectorListSerializer,
                CssSelectorListDeserializer,
            );
        });
    });
