...
import {
    ...
    PL_PARAM_START,
    PL_STRIDE,
} from '../../../src/parser/misc/parameter-list';
...
/**
 * Tokenize and parse a comma-separated parameter list (no surrounding parens).
 *
 ...
 */
function parse(input: string, listStart = 0, listEnd = input.length): Int32Array {
    tokenizer.setSource(input);
    initParserContext(ctx, input, tokenizer);
    ParameterListParser.parse(ctx, 0, tokenizer.tokenCount, listStart, listEnd, buf);
    return buf;
}
 ...
 *
 * @param data Buffer filled by ParameterListParser.parse.
