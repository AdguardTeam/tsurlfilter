...
import {
    DomainListGenerator,
} from '../../../src/generator/misc/domain-list-generator';
import {
    type DomainList,
...
describe('DomainListParser', () => {
    ...
    test('parse should work as expected on valid input', () => {
        ...
        // Multiple domains
        expect(
            DomainListParser.parse('example.com,example.net'),
        ).toEqual<DomainList>({
            type: ListNodeType.DomainList,
            start: 0,
            end: 23,
        ...
        expect(
            DomainListParser.parse('example.com,example.net,example.org'),
        ).toEqual<DomainList>({
            type: ListNodeType.DomainList,
            start: 0,
            end: 35,
    ...
    describe('parser options should work as expected', () => {
        test.each([
            ...
            expect(
                DomainListParser.parse(actual, { isLocIncluded: false }),
            ).toEqual(expected);
        });
    });
    ...
    test('generate', () => {
        const parseAndGenerate = (
            ...
            const ast = DomainListParser.parse(
                raw,
                defaultParserOptions,
                0,
                separator,
        ...
        expect(parseAndGenerate('example.com,example.org')).toEqual(
        ...
        expect(
            parseAndGenerate('~example.com,example.org,example.net'),
    ...
    describe('regex domain patterns with special characters', () => {
        test('should parse regex domains with commas correctly', () => {
            ...
            expect(
                DomainListParser.parse(regexWithAlternation),
            ).toEqual<DomainList>({
                type: ListNodeType.DomainList,
                start: 0,
