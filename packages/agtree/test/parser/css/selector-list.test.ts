/**
 * @file Parser-level unit tests for `SelectorListParser`.
 *
 * Each test tokenises a CSS selector string, runs
 * `SelectorListParser.parse()`, then inspects the flat `Int32Array`
...
/**
 * Tokenise `source` and run `SelectorListParser.parse()`.
 *
...
function p(source: string): void {
    ...
    SelectorListParser.parse(ctx, 0, ctx.tokenCount, DATA_OFFSET);
}
...
function child(i: number): {
    ...
    const base = SelectorListParser.childBase(
        DATA_OFFSET,
        DEFAULT_MAX_COMPLEX,
        i,
    );
...
describe('User Story 1 — Simple selectors', () => {
    test('type selector: div', () => {
        ...
        expect(SelectorListParser.childCountInComplex(ctx.data, DATA_OFFSET, 0)).toBe(1);
...
describe('User Story 2 — Compound selectors', () => {
    test('div#id.class', () => {
        p('div#id.class');
        expect(SelectorListParser.complexCount(ctx.data, DATA_OFFSET)).toBe(1);
        expect(SelectorListParser.childCountInComplex(ctx.data, DATA_OFFSET, 0)).toBe(3);
    ...
    test('[attr1="v1"][attr2]', () => {
        p('[attr1="v1"][attr2]');
        expect(SelectorListParser.childCountInComplex(ctx.data, DATA_OFFSET, 0)).toBe(2);
    ...
    test('div.class:hover[attr]', () => {
        ...
        expect(SelectorListParser.childCountInComplex(ctx.data, DATA_OFFSET, 0)).toBe(4);
...
describe('User Story 3 — Complex selectors with combinators', () => {
    ...
    test('div > span + a ~ h1 (three combinators)', () => {
        p('div > span + a ~ h1');
        expect(SelectorListParser.childCountInComplex(ctx.data, DATA_OFFSET, 0)).toBe(7);
...
describe('User Story 4 — Selector lists', () => {
    test('div, span', () => {
        p('div, span');
        expect(SelectorListParser.complexCount(ctx.data, DATA_OFFSET)).toBe(2);

        expect(SelectorListParser.childCountInComplex(ctx.data, DATA_OFFSET, 0)).toBe(1);
        expect(SelectorListParser.childCountInComplex(ctx.data, DATA_OFFSET, 1)).toBe(1);
    ...
    test('three complex selectors: div, .class, #id', () => {
        p('div, .class, #id');
        expect(SelectorListParser.complexCount(ctx.data, DATA_OFFSET)).toBe(3);
    });

    test('source ranges of complex selectors', () => {
        p('div, span');
        const srcStart0 = SelectorListParser.complexSourceStart(ctx.data, DATA_OFFSET, 0);
        const srcEnd0 = SelectorListParser.complexSourceEnd(ctx.data, DATA_OFFSET, 0);
        expect(ctx.source.slice(srcStart0, srcEnd0)).toBe('div');

        const srcStart1 = SelectorListParser.complexSourceStart(ctx.data, DATA_OFFSET, 1);
        const srcEnd1 = SelectorListParser.complexSourceEnd(ctx.data, DATA_OFFSET, 1);
...
describe('Edge cases', () => {
    ...
    test('all combinator types in one selector', () => {
        ...
        expect(SelectorListParser.childCountInComplex(ctx.data, DATA_OFFSET, 0)).toBe(9);
        expect(child(1).f0).toBe(COMBINATOR_CHILD);
        expect(child(3).f0).toBe(COMBINATOR_NEXT_SIBLING);
        expect(child(5).f0).toBe(COMBINATOR_SUBSEQUENT_SIBLING);
    ...
    test('childStartIndex for multiple complex selectors', () => {
        ...
        expect(SelectorListParser.childStartIndex(ctx.data, DATA_OFFSET, 0)).toBe(0);
        expect(SelectorListParser.childStartIndex(ctx.data, DATA_OFFSET, 1)).toBe(3);
        expect(SelectorListParser.childStartIndex(ctx.data, DATA_OFFSET, 2)).toBe(4);
    });
...
describe('User Story 6 — Error cases', () => {
    ...
    test('maxComplex=1: second selector via comma throws before write (regression)', () => {
        // parse "div, span" with maxComplex=1 — the comma triggers the
        // check; the second complex record must NOT be written.
        const source = 'div, span';
        tokenizer.setSource(source);
        initParserContext(ctx, source, tokenizer);
        expect(() => SelectorListParser.parse(
            ctx,
            0,
            ctx.tokenCount,
            DATA_OFFSET,
    ...
    test('maxComplex=1: single selector that is the second would throw (finalization path regression)', () => {
        // parse "div, span" with maxComplex=1 — same source, same expectation;
    ...
    test('maxComplex=1: single selector succeeds, two selectors throw (finalization guard)', () => {
        ...
        initParserContext(ctx, src1, tokenizer);
        expect(() => SelectorListParser.parse(
            ctx,
            0,
            ctx.tokenCount,
        ...
        initParserContext(ctx, src2, tokenizer);
        expect(() => SelectorListParser.parse(
            ctx,
            0,
            ctx.tokenCount,
