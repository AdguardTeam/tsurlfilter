...
} from '../../src/compatibility-tables/scriptlets';
...
describe('Scriptlets Compatibility Table', () => {
    it('scriptletsCompatibilityTable.existsAny', () => {
        expect(
            scriptletsCompatibilityTable.existsAny(
                'abort-current-inline-script',
            ),
        ).toBeTruthy();
        expect(
            scriptletsCompatibilityTable.existsAny('abort-on-property-read'),
        ).toBeTruthy();

        // prevent-innerHTML scriptlet
        expect(
            scriptletsCompatibilityTable.existsAny('prevent-innerHTML'),
        ).toBeTruthy();
        expect(
            scriptletsCompatibilityTable.existsAny('prevent-innerHTML.js'),
        ).toBeTruthy();

        expect(
            scriptletsCompatibilityTable.existsAny('nonexistent'),
        ).toBeFalsy();
    });
    ...
    it('scriptletsCompatibilityTable.getSingle', () => {
        expect(
            scriptletsCompatibilityTable.getSingle(
                'abort-current-inline-script',
                SpecificPlatform.AdgExtChrome,
            ),
    ...
    it('scriptletsCompatibilityTable.getMultiple', () => {
        expect(
            scriptletsCompatibilityTable.getMultiple(
                'abort-current-inline-script',
                GenericPlatform.AdgExtAny,
            ),
        ).toMatchObject({
