...
import { DomainListAstBuilder } from '../../../src/ast-builder/misc/domain-list';
import { createParserContext, domainRecordsOffset, initParserContext } from '../../../src/parser/context';
import { CR_DOMAIN_COUNT } from '../../../src/parser/cosmetic/constants';
...
describe('DomainListParser', () => {
    ...
    describe('parse (with location)', () => {
        test('single domain', () => {
            ...
            DomainListParser.parse(ctx, 0, ctx.tokenCount, TokenType.Comma);
            const domainCount = ctx.data[CR_DOMAIN_COUNT];

            const domainList = DomainListAstBuilder.parse(
                source,
                ctx.data,
                domainCount,
        ...
        test('domains separated by pipe', () => {
            ...
            DomainListParser.parse(ctx, 0, ctx.tokenCount, TokenType.Pipe);
            const domainCount = ctx.data[CR_DOMAIN_COUNT];
