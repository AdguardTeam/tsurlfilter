...
describe('PositionProvider', () => {
    describe('convertOffsetToPosition', () => {
        test('should work for valid offsets', () => {
            const fixture = fixtureMatrix
                .map((line) => line.join(EMPTY))
                .join(LF);
            ...
            for (let i = 0; i < fixtureMatrix.length; i += 1) {
                for (let j = 0; j < fixtureMatrix[i].length; j += 1) {
                    expect(
                        provider.convertOffsetToPosition(
                            fixture.indexOf(fixtureMatrix[i][j]),
                        ),
                    ).toEqual({
                        line: i + 1,
                        column: j + 1,
        ...
        test('should return null for invalid offsets', () => {
            ...
            expect(
                provider.convertOffsetToPosition(fixture.length + 1),
            ).toBeNull();
        });
    });
});
