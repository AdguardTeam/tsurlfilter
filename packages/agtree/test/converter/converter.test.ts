...
/**
 * Helper function to convert a rule to AdGuard format. It makes easier to test rules as strings.
 *
 * @param rule Rule to convert
 * @returns Array of converted rules
 */
const convertToAdgEx = (rule: string) => {
    return RuleConverter.convertToAdg(RuleParser.parse(rule)).map(RuleParser.generate);
};

/**
 * Helper function to convert a value to an array. If the value is already an array, it is returned as-is.
 * Just for convenience, because often we just have a single conversion result, but converter always returns an array.
 *
    ...
    describe('should convert rules to ADG', () => {
        ...
        describe('should convert uBO comments', () => {
            test.each([
                ...
                expect(convertToAdgEx(actual)).toEqual(toArray(expected));
            });
        });
        ...
        describe('should throw error on invalid uBO HTML filtering rules', () => {
            test.each([
                ...
                expect(() => convertToAdgEx(actual)).toThrowError(
        ...
        describe('should convert redirect resources', () => {
            ...
            ])('should throw \'$expected\' when converting \'$actual\'', ({ actual, expected }) => {
                expect(() => convertToAdgEx(actual)).toThrowError(new RuleConversionError(expected));
            });
        });
    });
});
