...
export class QuoteUtils {
     ...
     */
    public static escapeUnescapedOccurrences(
        string: string,
        char: string,
    ): string {
        let result = EMPTY;
        ...
        for (let i = 0; i < string.length; i += 1) {
            if (
                string[i] === char
                && (i === 0 || string[i - 1] !== ESCAPE_CHARACTER)
            ) {
     ...
     */
    public static unescapeSingleEscapedOccurrences(
        string: string,
        char: string,
    ): string {
        let result = EMPTY;
    ...
    public static getStringQuoteType(string: string): QuoteType {
        ...
        if (string.length > 1) {
            if (
                string.startsWith(SINGLE_QUOTE)
                && string.endsWith(SINGLE_QUOTE)
            ) {
                return QuoteType.Single;
            }

            if (
                string.startsWith(DOUBLE_QUOTE)
                && string.endsWith(DOUBLE_QUOTE)
            ) {
                return QuoteType.Double;
            }

            if (
                string.startsWith(BACKTICK_QUOTE)
                && string.endsWith(BACKTICK_QUOTE)
            ) {
     ...
     */
    public static setStringQuoteType(
        string: string,
        quoteType: QuoteType,
    ): string {
            ...
            case QuoteType.Single:
                ...
                if (actualQuoteType === QuoteType.Double) {
                    return (
                        SINGLE_QUOTE
                        + QuoteUtils.escapeUnescapedOccurrences(
                            QuoteUtils.unescapeSingleEscapedOccurrences(
                                string.slice(1, -1),
                                DOUBLE_QUOTE,
                            ),
                            SINGLE_QUOTE,
    ...
    public static removeQuotes(string: string): string {
        if (
            ...
            string.length > 1
            && (string[0] === SINGLE_QUOTE
                || string[0] === DOUBLE_QUOTE
                || string[0] === BACKTICK_QUOTE)
