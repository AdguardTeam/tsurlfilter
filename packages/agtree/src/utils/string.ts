...
export class StringUtils {
    ...
    public static splitStringByUnquotedUnescapedCharacter(
        ...
        do {
            ...
            if (delimeterIndex !== -1) {
                parts.push(
                    pattern.substring(prevDelimeterIndex + 1, delimeterIndex),
                );
            } else {
                parts.push(
                    pattern.substring(prevDelimeterIndex + 1, pattern.length),
                );
            }
    ...
    public static isLetter(char: string): boolean {
        return (
            StringUtils.isSmallLetter(char) || StringUtils.isCapitalLetter(char)
        );
    }
     ...
     * @returns Index or -1 if the character not found.
     */
    public static findFirstNonWhitespaceCharacter(
        pattern: string,
        start = 0,
    ): number {
        for (let i = start; i < pattern.length; i += 1) {
     ...
     */
    public static findNextWhitespaceCharacter(
        pattern: string,
        start = 0,
    ): number {
        for (let i = start; i < pattern.length; i += 1) {
