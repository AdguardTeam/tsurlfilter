...
export class LogicalExpressionUtils {
    ...
    public static getVariables(
        ...
        if (node.type === NodeType.Operator) {
            ...
            const rightVars = node.right
                ? LogicalExpressionUtils.getVariables(node.right)
                : [];
    ...
    public static evaluate(
        ...
        if (node.type === NodeType.Operator) {
            if (
                node.operator === OperatorValue.And
                || node.operator === OperatorValue.Or
            ) {
                if (!node.right) {
                    throw new Error(
                        `${ERROR_PREFIX.UNEXPECTED_OPERATOR} '${node.operator}'`,
                    );
                }
