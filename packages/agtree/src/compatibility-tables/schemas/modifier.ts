 ...
 */
export const modifierDataSchema = zodToCamelCase(
    baseCompatibilityDataSchema
        .extend({
            /**
            ...
            /**
             * Describes whether the actual modifier supports value assignment. For example,
             * `$domain` is assignable, so it can be used like this:
             * `$domain=domain.com\|~subdomain.domain.com`, where `=` is the assignment
             * operator and `domain.com\|~subdomain.domain.com` is the value.
             */
             ...
             * Its value can be a regex pattern or a known validator name
             * (e.g. `domain`, `pipe_separated_domains`, etc.).
             */
        ...
        .superRefine((data, ctx) => {
            ...
            if (data.block_only && data.exception_only) {
                ctx.addIssue({
                    ...
                    message:
                        'block_only and exception_only are mutually exclusive',
                });
            }
            ...
            if (data.assignable && !data.value_format) {
                ctx.addIssue({
                    code: zod.ZodIssueCode.custom,
                    message:
                        'value_format is required for assignable modifiers',
                });
            }
                ...
                if (KNOWN_VALIDATORS.has(valueFormat)) {
                    if (data.value_format_flags) {
                        ctx.addIssue({
                            code: zod.ZodIssueCode.custom,
                            message:
                                'value_format_flags are not allowed for known validators',
                        });
                    }
                    return;
                }
