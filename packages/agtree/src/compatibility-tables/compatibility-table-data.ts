...
const getModifiersCompatibilityTableData = (dir: string) => {
    const valueFormatPreprocessorSchema = zod
        .object({
        ...
        .transform((data) => {
            ...
            if (
                !valueFormatTrimmed
                && KNOWN_VALIDATORS.has(valueFormatTrimmed)
            ) {
                return data;
            }
            ...
            const regExpPattern = new RegExp(
                xRegExpPattern.source,
                xRegExpPattern.flags,
            );
    ...
    const data = getCompatibilityTableData<ModifierDataSchema>(
        dir,
        baseFileSchema(combinedSchema),
    );
...
const getRedirectsCompatibilityTableData = (dir: string) => {
    return getCompatibilityTableData<RedirectDataSchema>(
        dir,
        baseFileSchema(redirectDataSchema),
    );
};
...
const getScriptletsCompatibilityTableData = (dir: string) => {
    return getCompatibilityTableData<ScriptletDataSchema>(
        dir,
        baseFileSchema(scriptletDataSchema),
    );
};
