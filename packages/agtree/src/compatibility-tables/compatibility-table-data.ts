...
const getYmlFilesFromDir = (dir: string): string[] => {
    ...
    return dirEntries.filter((dirent) => dirent.isFile() && dirent.name.endsWith('.yml')).map((dirent) => dirent.name);
};
...
const getModifiersCompatibilityTableData = (dir: string) => {
    const valueFormatPreprocessorSchema = zod
        ...
        .passthrough()
        .transform((data) => {
            const { value_format: valueFormat, value_format_flags: valueFormatFlags } = data;
