...
import { GenericPlatform, type SpecificPlatformType } from './platforms';
...
export abstract class CompatibilityTableBase<T extends BaseCompatibilityDataSchema> {
     ...
     * @returns True if the compatibility data exists, false otherwise.
     */
    public exists(name: string, platform: SpecificPlatformType | GenericPlatform): boolean {
        const normalizedName = this.nameTransformer ? this.nameTransformer(name) : name;
     ...
     */
    public getAllMultiple(platform: SpecificPlatformType | GenericPlatform): SinglePlatformRecords<T>[] {
        const result: SinglePlatformRecords<T>[] = [];
     ...
     */
    public getFirst(name: string, platform: SpecificPlatformType | GenericPlatform): T | null {
        const normalizedName = this.nameTransformer ? this.nameTransformer(name) : name;
        const data = this.getRowStorage(normalizedName);
