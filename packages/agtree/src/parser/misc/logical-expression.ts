...
/**
 * @file Logical expression parser.
 *
...
/**
 * Token-based parser for logical expressions used in preprocessor
 * directives (`!#if`, `!#endif`, …).
 *
 * Call {@link LogicalExpressionParser.parse} to fill an `Int32Array`
...
export class LogicalExpressionParser {
     ...
     *
     * @throws When the expression is syntactically invalid.
     */
    public static parse(
        ctx: ParserContext,
        startTi: number,
        ...
        const root = LogicalExpressionParser.parseExpr(ctx, endTi, buf, cursor, 0);
     ...
     *
     * @param buf Buffer written by {@link LogicalExpressionParser.parse}.
     *
    ...
    private static parseExpr(
        ...
        let node = LogicalExpressionParser.parsePrimary(ctx, endTi, buf, cursor);
        ...
        while (cursor.ti < endTi) {
            ...
            const right = LogicalExpressionParser.parseExpr(ctx, endTi, buf, cursor, prec + 1);

            const leftStart = buf[LE_HEADER + node * LE_STRIDE + LE_SRC_START];
            const rightEnd = buf[LE_HEADER + right * LE_STRIDE + LE_SRC_END];
            node = LogicalExpressionParser.alloc(buf, cursor, opKind, leftStart, rightEnd, node, right);
    ...
    private static parsePrimary(
        ...
        if (types[ti] === TokenType.ExclamationMark) {
            ...
            cursor.ti += 1;
            const child = LogicalExpressionParser.parseExpr(ctx, endTi, buf, cursor, PREC_NOT);
            const childEnd = buf[LE_HEADER + child * LE_STRIDE + LE_SRC_END];
            return LogicalExpressionParser.alloc(buf, cursor, LE_KIND_NOT, opStart, childEnd, child, -1);
        }
        ...
        if (types[ti] === TokenType.OpenParen) {
            // skip `(`
            cursor.ti += 1;
            const inner = LogicalExpressionParser.parseExpr(ctx, endTi, buf, cursor, 0);
            ...
            return LogicalExpressionParser.alloc(buf, cursor, LE_KIND_PAR, innerStart, innerEnd, inner, -1);
        }
        ...
        if ((IDENT_START_MASK >>> types[ti]) & 1) {
            ...
            return LogicalExpressionParser.alloc(buf, cursor, LE_KIND_VAR, varStart, varEnd, -1, -1);
        }
