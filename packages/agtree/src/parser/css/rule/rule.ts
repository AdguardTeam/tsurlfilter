...
/**
 * @file CSS qualified rule parser.
 *
 * `CssRuleParser.parse()` scans adblock tokens to find the structural
 ...
 * means selector tokens are never touched by the rule parser — they are
 * only analyzed once by `SelectorListParser` during the AST build phase.
 *
