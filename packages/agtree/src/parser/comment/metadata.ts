...
/**
 * @file Metadata comment parser.
 *
 * Handles `! Header: value` and `# Header: value` rules.
 * Records the marker, header name, and value bounds in `ctx.data`.
 *
 * ## Data Layout
...
export class MetadataCommentParser implements StructuralParser {
    ...
    public static parse(ctx: ParserContext, startTi = 0, endTi = ctx.tokenCount, dataOffset = 0): void {
        ...
        if (ti < endTi && ctx.types[ti] === TokenType.Whitespace) {
            ...
            if (ti < endTi && ctx.types[ti] !== TokenType.Colon) {
        ...
        const lastTi = lastNonWs(ctx, valueTi, endTi);
        const valueEnd = lastTi >= 0 ? ctx.ends[lastTi] : valueStart;

        data[dataOffset + CM_KIND] = CommentKind.Metadata;
        data[dataOffset + CM_META_MARKER_OFFSET] = markerStart;
        data[dataOffset + CM_META_HEADER_START_OFFSET] = headerStart;
