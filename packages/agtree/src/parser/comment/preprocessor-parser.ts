...
export class PreProcessorCommentParser extends BaseParser {
    ...
    public static isPreProcessorRule(raw: string): boolean {
        ...
        return (
            trimmed.startsWith(PREPROCESSOR_MARKER)
            && trimmed[PREPROCESSOR_MARKER_LEN] !== HASHMARK
        );
    }
    ...
    public static parse(
        ...
        // Create name node
        const name = ValueParser.parse(
            raw.slice(nameStart, nameEnd),
            options,
