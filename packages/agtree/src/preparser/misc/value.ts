...
export class ValuePreparser {
     ...
     */
    public static parseStandard(ctx: PreparserContext, ti: number, end: number): number {
     ...
     * @returns Token index at the separator comma or `end`.
     */
    public static parseReplace(ctx: PreparserContext, ti: number, end: number): number {
        const { types } = ctx;
