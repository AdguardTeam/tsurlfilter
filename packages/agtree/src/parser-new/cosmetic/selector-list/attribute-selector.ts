 ...
 *
 * Builds an AttributeSelector AST node from a preparsed child record.
...
import {
    ...
    NO_VALUE,
} from '../../../preparser/css/selector-list/constants';
import { ValueParser } from '../../misc/value';

/**
 * Build an AttributeSelector AST node from a preparsed child record.
 *
 ...
 *
 * For quoted attribute values the preparser stores offsets that already
 * exclude the surrounding quote characters, so `source.slice()` yields the
 * unquoted content directly.
 *
 * @param source Original source string.
 * @param data Int32Array buffer with preparsed data.
...
export function buildAttributeSelector(
    ...
    const operator = ValueParser.parse(
        source,
        operatorStart,
        operatorEnd,
        isLocIncluded,
    ...
    const value = ValueParser.parse(source, valueStart, valueEnd, isLocIncluded);
