 ...
 *
 * Builds a PseudoClassSelector AST node from a preparsed child record.
 */
...
/**
 * Build a PseudoClassSelector AST node from a preparsed child record.
 *
...
export function buildPseudoClassSelector(
    ...
    const node: PseudoClassSelector = {
        ...
        name: ValueParser.parse(source, nameStart, nameEnd, isLocIncluded),
    };

    if (argStart !== NO_VALUE) {
        const argEnd = data[base + CHILD_FIELD_3];
        node.argument = ValueParser.parse(source, argStart, argEnd, isLocIncluded);
    }
