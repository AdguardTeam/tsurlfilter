 ...
 * `SelectorListPreparser.preparse()`) and builds a SelectorList AST node
...
import { CHILD_FIELD_KIND, ChildKind, DEFAULT_MAX_COMPLEX } from '../../../preparser/css/selector-list/constants';
 ...
 * `SelectorList` AST node. Performs **zero validation** — the preparser
 * guarantees structural correctness before this builder is ever called.
 */
export class SelectorListAstParser {
    /**
     * Build a SelectorList AST node from preparsed data.
     *
     * @param source Original source string (the raw selector substring).
     * @param data Int32Array that was written by `SelectorListPreparser.preparse()`.
     * @param dataOffset Base offset within `data` where selector-list data starts.
     * @param maxComplex Maximum complex selector capacity that was used during preparse.
    ...
    public static parse(
        ...
        const complexCount = SelectorListPreparser.complexCount(data, dataOffset);
        const complexSelectors: ComplexSelector[] = new Array(complexCount);

        for (let ci = 0; ci < complexCount; ci += 1) {
            const childCount = SelectorListPreparser.childCountInComplex(data, dataOffset, ci);
            const childStart = SelectorListPreparser.childStartIndex(data, dataOffset, ci);
            ...
            for (let j = 0; j < childCount; j += 1) {
                ...
                const base = SelectorListPreparser.childBase(dataOffset, maxComplex, globalChildIdx);
            ...
            if (isLocIncluded) {
                complexNode.start = SelectorListPreparser.complexSourceStart(data, dataOffset, ci);
                complexNode.end = SelectorListPreparser.complexSourceEnd(data, dataOffset, ci);
            }
