...
export class AdgScriptletInjectionBodyGenerator extends BaseGenerator {
    ...
    public static readonly ERROR_MESSAGES = {
        NO_MULTIPLE_SCRIPTLET_CALLS:
            'ADG syntaxes does not support multiple scriptlet calls within one single rule',
    };
    ...
    public static generate(node: ScriptletInjectionRuleBody): string {
        ...
        if (node.children.length > 1) {
            throw new Error(
                AdgScriptletInjectionBodyGenerator.ERROR_MESSAGES.NO_MULTIPLE_SCRIPTLET_CALLS,
            );
        }
