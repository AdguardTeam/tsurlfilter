...
export class UboScriptletInjectionBodyGenerator extends BaseGenerator {
    ...
    public static readonly ERROR_MESSAGES = {
        NO_MULTIPLE_SCRIPTLET_CALLS:
            'uBO syntaxes does not support multiple scriptlet calls within one single rule',
    };
    ...
    public static generate(node: ScriptletInjectionRuleBody): string {
        ...
        if (node.children.length > 1) {
            throw new Error(
                UboScriptletInjectionBodyGenerator.ERROR_MESSAGES.NO_MULTIPLE_SCRIPTLET_CALLS,
            );
        }
