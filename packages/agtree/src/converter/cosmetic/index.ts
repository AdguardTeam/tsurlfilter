    ...
    public static convertToAdg(
        ...
        if (rule.modifiers) {
            if (rule.syntax === AdblockSyntax.Ubo) {
                ...
                convertedModifiers = AdgCosmeticRuleModifierConverter.convertFromUbo(
                    rule.modifiers,
                );
            } else if (rule.syntax === AdblockSyntax.Abp) {
                // TODO: Implement once ABP starts supporting cosmetic rule modifiers
                throw new RuleConversionError(
                    "ABP don't support cosmetic rule modifiers",
                );
            }
        }

        if (
            subconverterResult.result.length > 1
            ...
            subconverterResult.result.forEach((subconverterRule) => {
                if (
                    convertedModifiers
                    && subconverterRule.category === RuleCategory.Cosmetic
                ) {
    ...
    public static convertToUbo(
        ...
        switch (rule.type) {
            ...
            case CosmeticRuleType.ElementHidingRule: {
                ...
                const isElementHidingRule = rule.separator.value
        ...
        if (rule.modifiers) {
            if (rule.syntax === AdblockSyntax.Abp) {
                // TODO: Implement once ABP starts supporting cosmetic rule modifiers
                throw new RuleConversionError(
                    'ABP does not support cosmetic rule modifiers',
                );
            } else if (rule.syntax === AdblockSyntax.Adg) {
                convertedModifiers = UboCosmeticRuleModifierConverter.convertFromAdg(
                    rule.modifiers,
                );
            }
        }
