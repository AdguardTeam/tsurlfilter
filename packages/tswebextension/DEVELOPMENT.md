...
Before working on `tswebextension`, build the packages it depends on:
...
This cleans `dist/`, emits type declarations, bundles via Rollup (ESM bundles +
content scripts), and builds metadata.
...
```bash
pnpm --filter @adguard/tswebextension start
```
...
# Type-check
pnpm --filter @adguard/tswebextension lint:types
...
# Run MV2 tests only
pnpm --filter @adguard/tswebextension test:mv2

# Run MV3 tests only
pnpm --filter @adguard/tswebextension test:mv3

# Run common tests only
pnpm --filter @adguard/tswebextension test:common

# Run tests with V8 coverage
pnpm --filter @adguard/tswebextension test:coverage
...
All commands can also be run from the `packages/tswebextension/` directory
