...
describe('getRuleHashFromFilePath', () => {
    it('extracts the hash from a per-rule file path', () => {
        expect(getRuleHashFromFilePath('filters/preregistered-scripts/deadbeefdeadbeef.js'))
            .toBe('deadbeefdeadbeef');
        expect(getRuleHashFromFilePath('deadbeefdeadbeef.js')).toBe('deadbeefdeadbeef');
    });

    it('round-trips with getRuleFilename', async () => {
        const hash = await hashString('j:some-rule-body');
        expect(getRuleHashFromFilePath(`any/dir/${getRuleFilename(hash)}`)).toBe(hash);
    });

    it('returns null for non-rule files', () => {
        expect(getRuleHashFromFilePath('filters/preregistered-scripts/scriptlets-bundle.js')).toBeNull();
        expect(getRuleHashFromFilePath('filters/preregistered-scripts/cleanup.js')).toBeNull();
        expect(getRuleHashFromFilePath('filters/preregistered-scripts/s-deadbeefdeadbeef.js')).toBeNull();
    });

    it('returns null for malformed hashes', () => {
        // Too short / too long / non-hex characters.
        expect(getRuleHashFromFilePath('dir/deadbeef.js')).toBeNull();
        expect(getRuleHashFromFilePath('dir/deadbeefdeadbeef0.js')).toBeNull();
        expect(getRuleHashFromFilePath('dir/DEADBEEFDEADBEEF.js')).toBeNull();
        expect(getRuleHashFromFilePath('dir/zzzzzzzzzzzzzzzz.js')).toBeNull();
    });
});
...
describe('normalizeDomain', () => {
    ...
    it('keeps a leading www prefix (apex and www are distinct hosts)', () => {
        expect(normalizeDomain('www.youtube.com')).toBe('www.youtube.com');
    });
