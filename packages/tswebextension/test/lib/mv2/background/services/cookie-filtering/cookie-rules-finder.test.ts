...
describe('Cookie rules - lookup rules', () => {
    it('looks up blocking rules', () => {
        let rule = CookieRulesFinder.lookupNotModifyingRule('test', [createNetworkRule('$cookie=test', 0)], false);
        expect(rule).not.toBeNull();

        rule = CookieRulesFinder.lookupNotModifyingRule('not_test', [createNetworkRule('$cookie=test', 0)], false);
        expect(rule).toBeNull();

        rule = CookieRulesFinder.lookupNotModifyingRule(
            'test',
            [createNetworkRule('$cookie=not_test', 0), createNetworkRule('$cookie=test', 0)],
            false,
        );
        expect(rule).not.toBeNull();
    ...
    it('looks up blocking rules - third-party cookie', () => {
        let rule = CookieRulesFinder.lookupNotModifyingRule('test', [createNetworkRule('$cookie=test', 0)], true);
        expect(rule).not.toBeNull();
    ...
    it('looks up blocking rules - cookie specific allowlist', () => {
        let rule = CookieRulesFinder.lookupNotModifyingRule(
            'pick',
            [createNetworkRule('$cookie=pick', 0), createNetworkRule('@@||example.org^$cookie=pick', 0)],
            true,
        );
        expect(rule).not.toBeNull();
    ...
    it('looks up modifying rules', () => {
        let rules = CookieRulesFinder.lookupModifyingRules('test', [createNetworkRule('$cookie=test', 0)], false);
        expect(rules).toHaveLength(0);

        rules = CookieRulesFinder.lookupModifyingRules(
            'test',
            [createNetworkRule('$cookie=test;maxAge=15;sameSite=lax', 0)],
            false,
        );
    ...
    it('looks up modifying rules - cookie specific allowlist', () => {
        const rule = CookieRulesFinder.lookupModifyingRules(
            'pick',
            [
