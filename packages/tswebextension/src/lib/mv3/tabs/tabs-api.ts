...
export class TabsApi {
     ...
     */
    public getTabFrame(tabId: number, frameId: number): FrameMV3 | null {
        const tabContext = this.context.get(tabId);
     ...
     * @returns Frame data or null if not found.
     */
    public getTabMainFrame(tabId: number): FrameMV3 | null {
     ...
     */
    public setFrameContext(tabId: number, frameId: number, frameContext: FrameMV3): void {
        const tabContext = this.getTabContext(tabId);
        if (!tabContext) {
            logger.debug('At this point tab context should already exist');
            return;
     ...
     */
    public getFrameContext(tabId: number, frameId: number): FrameMV3 | undefined {
        const tabContext = this.getTabContext(tabId);
        const frameContext = tabContext?.getFrameContext(frameId);
        return frameContext;
    }
     ...
     * @returns Frame context.
     */
    public getByDocumentId(tabId: number, documentId: string): FrameMV3 | undefined {
        const tabContext = this.getTabContext(tabId);
