...
import { type ConfigurationMV2, type ConfigurationResultMV2 } from './configuration';
...
export class EngineApi {
     ...
     */
    public async startEngine(configuration: ConfigurationMV2): Promise<ConfigurationResultMV2> {
        const {
            filters,
            userrules,
            verbose,
        ...
        for (let i = 0; i < filters.length; i += 1) {
            ...
            const filterList = conversionData ? new FilterList(content, filterId, conversionData) : content;
        ...
        if (userrules.content.length > 0) {
            // TODO: AG-54975 Refactor FilterList constructor to treat empty ConversionData as undefined,
            // so callers that guarantee AdGuard-compatible content can opt out of prepare() explicitly.
            // Then this guard can be replaced with:
            // new FilterList(userrules.content, USER_FILTER_ID, userrules.conversionData).
            const conversionData = userrules.conversionData?.originals.length ? userrules.conversionData : undefined;
            const userFilterList = new FilterList(userrules.content, USER_FILTER_ID, conversionData);

            lists.push({
                id: USER_FILTER_ID,
                content: userFilterList,
                ignoreCosmetic: false,
                ignoreJS: false,
                ignoreUnsafe: false,
            });
        ...
        */
        ...
        return {
            conversionErrors: engine.getConversionErrors(),
        };
    }
