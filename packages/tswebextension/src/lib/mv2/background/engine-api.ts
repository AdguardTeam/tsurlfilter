...
export class EngineApi {
    ...
    public async startEngine(configuration: ConfigurationMV2): Promise<void> {
        ...
        for (let i = 0; i < filters.length; i += 1) {
            ...
            lists.push({
                id: filterId,
                content: conversionData ? new ConvertedFilterList(content, conversionData) : content,
        ...
        if (userrules.content.length > 0) {
            lists.push({
                ...
                content: userrules.conversionData
                    ? new ConvertedFilterList(userrules.content, userrules.conversionData)
                    : userrules.content,
                ignoreCosmetic: false,
                ignoreJS: false,
                ignoreUnsafe: false,
        ...
        if (allowlistRulesList) {
            const allowlistContent = new ConvertedFilterList(
                allowlistRulesList,
        ...
        if (stealthModeList) {
            const stealthContent = new ConvertedFilterList(
                stealthModeList,
                // this filter list is generated by tswebextension, so its guaranteed to be AdGuard compatible
                ConvertedFilterList.createEmptyConversionData(),
            );
