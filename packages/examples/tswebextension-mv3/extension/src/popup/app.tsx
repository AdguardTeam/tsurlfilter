...
export function App() {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { status, filters, rules } = (await sendMessage(Message.GetConfig)) as ConfigResponse;
  ...
  const onToggle = async () => {
    ...
    const message = isFilteringEnabled ? Message.TurnOff : Message.TurnOn;
  ...
  const toggleBtn = (
    <button type="button" className="button" disabled={isLoading} onClick={onToggle}>
      <span className="text">{isFilteringEnabled ? 'Turn off' : 'Turn on'}</span>
    </button>
  );
  ...
  const filters = filtersList.map((id) => {
    ...
    const toggleFilter = () => {
      const updatedIds = checked ? enabledFilters.filter((i) => i !== id) : enabledFilters.concat(id);
  ...
  const assistantPanel = (
    <>
      <button className="button" type="button" onClick={handleOpenAssistant}>
        <span className="text">Open assistant</span>
      </button>
      <button className="button" type="button" onClick={handleCloseAssistant}>
        <span className="text">Close assistant</span>
      </button>
    </>
  );
