...
const rulesTextToArray = (text: string): string[] =>
  text
    .trim()
    .split('\n')
    .filter((el) => el !== '');
    ...
    >
      {({ values, handleChange, handleSubmit }) => (
        <form className="form" onSubmit={handleSubmit}>
          ...
          <textarea rows={10} name="userrules" id="userrules" onChange={handleChange} value={values.userrules} />
          <label>Allowlist</label>
          <textarea rows={10} name="allowlist" id="allowlist" onChange={handleChange} value={values.allowlist} />
