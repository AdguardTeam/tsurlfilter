...
echo "css-tokenizer-benchmark"

# import helper functions and some common variables
. ./bamboo-specs/scripts/helpers.sh

if [ "$branch" != "master" ] && ! is_root_affected && ! is_project_affected "css-tokenizer-benchmark"; then
  echo "No changes in css-tokenizer-benchmark, skipping tests"
  exit 0;
fi
...
npx lerna run build --include-dependencies --scope @adguard/css-tokenizer

# Lint
pnpm --filter css-tokenizer-benchmark lint

echo "css-tokenizer-benchmark tests completed"
