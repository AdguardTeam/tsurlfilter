...
echo "agtree-browser-benchmark"

# import helper functions and some common variables
. ./bamboo-specs/scripts/helpers.sh

if [ "$branch" != "master" ] && ! is_root_affected && ! is_project_affected "agtree-browser-benchmark"; then
  echo "No changes in agtree-browser-benchmark, skipping tests"
  exit 0;
fi
...
# Lint
pnpm --filter agtree-browser-benchmark lint

echo "agtree-browser-benchmark tests completed"
